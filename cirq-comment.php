...
if (!isset($_SESSION["usermast"])) {
	...
	if (!empty($_POST["comment"])) {
		...
		if (in_array($_SESSION['user_id'], $authusers)) {
			...
			if (!isset($error)) {
				$insert = "insert into cirq_comments (circuit_id, user_mast_id, right_now, comment) values (:circuit_id, :user_mast_id, :right_now, :comment)";
				$insertparams = array(':circuit_id' => $circuit_id, ':user_mast_id' => $user_mast_id, ':right_now' => $right_now, ':comment' => $comment);
				try {
...
?>
		...
		<strong>located at: <?php print $area; ?></strong><div class="brbr"></div>
	        ...
	        <form method="post" action="cirq-comment.php" id="newlist">
		<div style="width:auto; float:left;">
			<div style="margin-top:145px;"><textarea name="comment" placeholder="comment on <?php print $crow['circuit']; ?>" style="font-size:.9em;"></textarea></div>
		</div>
		<div id="ncrightlist">
